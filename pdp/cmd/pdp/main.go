...
func main() {
	...
	if cfg.TLSCert == "" || cfg.TLSKey == "" {
		log.Fatal("[SECURITY] tls_cert and tls_key are required because gateway-to-PDP communication is strictly mTLS")
	}
	if cfg.MTLSCA == "" {
		log.Fatal("[SECURITY] mtls_ca is required because gateway-to-PDP communication is strictly mTLS")
	}
	...
	if strings.TrimSpace(cfg.PKIRoleDevice) == "" || strings.TrimSpace(cfg.PKIRoleGateway) == "" {
		log.Fatal("[SECURITY] pki_role_device and pki_role_gateway must be configured")
	}
	...
	// ──────────────────────────────
	registerOIDCClients(policyAdmin, cfg)
...
// registerOIDCClients registers native endpoint public clients.
// Gateways no longer act as OIDC clients and do not receive client secrets.
func registerOIDCClients(policyAdmin *pa.PolicyAdministrator, cfg *config.Config) {
	policyAdmin.IdP.OIDC.RegisterNativeConnectAppClient()
	policyAdmin.IdP.OIDC.RegisterNativeAgentClient()
	log.Printf("[OIDC] Registered native public client %q", idp.NativeConnectAppClientID)
	log.Printf("[OIDC] Registered native public client %q", idp.NativeAgentClientID)
}
...
func ensureTestUser(policyAdmin *pa.PolicyAdministrator) {
	...
	if err := policyAdmin.IdP.Users.SetUserRole(user.ID, "admin"); err != nil {
...
func printStatus(cfg *config.Config, s *store.Store) {
	...
	log.Println("──────────────────────────────────────────")
	log.Printf("  Service:  ZTNA PDP (PA + PE + IdP)")
	...
	log.Println("    POST /api/gateway/enroll        - One-time Gateway enrollment")
	log.Println("    POST /api/gateway/renew-cert    - Gateway certificate renewal")
	log.Println("    GET  /api/gateway/revoked-serials - Gateway revocation sync")
