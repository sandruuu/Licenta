...
func TestDirectoryStoreUsersGroupsAndMemberships(t *testing.T) {
	...
	if found, ok := s.FindDirectoryUserByExternalID("organization-1", "idp-1", "external-user-1"); !ok || found.ID != user.ID {
		t.Fatalf("FindDirectoryUserByExternalID mismatch: user=%+v ok=%v", found, ok)
	}
	if found, ok := s.FindDirectoryUserByUserName("organization-1", "idp-1", "alice@example.test"); !ok || found.ID != user.ID {
	...
	if err := s.ReplaceDirectoryGroupMembers("organization-1", "idp-1", group.ID, []string{user.ID}, now); err != nil {
		t.Fatalf("ReplaceDirectoryGroupMembers returned error: %v", err)
	}

	members := s.ListDirectoryGroupMembers("organization-1", "idp-1", group.ID)
	if len(members) != 1 || members[0].UserID != user.ID {
		t.Fatalf("members = %+v, want one member %s", members, user.ID)
	}
	groups := s.ListDirectoryGroupsForUser("organization-1", "idp-1", user.ID)
	if len(groups) != 1 || groups[0].ID != group.ID {
		t.Fatalf("groups for user = %+v, want %s", groups, group.ID)
	}

	if err := s.RemoveDirectoryGroupMembers("organization-1", "idp-1", group.ID, []string{user.ID}); err != nil {
		t.Fatalf("RemoveDirectoryGroupMembers returned error: %v", err)
	}
	if members := s.ListDirectoryGroupMembers("organization-1", "idp-1", group.ID); len(members) != 0 {
