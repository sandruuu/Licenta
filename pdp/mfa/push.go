...
type PushProvider struct {
	...
	// OnCreate, if non-nil, is invoked synchronously after a push
	// challenge has been persisted. The API server uses this to publish
	// a fan-out event so a device-health app subscribed via SSE
	// receives the challenge immediately instead of polling.
	OnCreate func(*models.PushChallenge)
}
