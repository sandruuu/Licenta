...
var (
	ErrServiceUnavailable  = errors.New("device telemetry service is not available")
	...
	ErrNoPriorPosture      = errors.New("no prior posture report on file")
)
...
func (service *Service) RecordPosture(report *models.DevicePostureReport) {
	if service == nil || service.store == nil || report == nil {
		return
	}
	service.store.SaveDevicePosture(report)
	if service.audit != nil {
		service.audit.LogEvent("device_posture_report", "", "", "", report.DeviceID,
			"", "Raw device posture reported", true)
	}
	log.Printf("[PA] Device posture report received: device=%s checks=%d", report.DeviceID, len(report.Checks))
}
...
func (service *Service) AcceptPostureReport(certDeviceID string, report models.DevicePostureReport) (models.DevicePostureReport, error) {
	if err := service.ready(); err != nil {
		return report, err
	}
	certDeviceID = strings.TrimSpace(certDeviceID)
	...
	if report.DeviceID != certDeviceID {
		log.Printf("[PA] Device posture report rejected: device_id=%q does not match cert CN=%q", report.DeviceID, certDeviceID)
		return report, ErrDeviceIDMismatch
	}
	...
	service.RecordPosture(&report)
	service.publish(events.TopicHealthChanged, map[string]string{
		"device_id": report.DeviceID,
		"reason":    "device_posture_reported",
	})
	return report, nil
}
...
func (service *Service) TouchPostureHeartbeat(deviceID string) (time.Time, error) {
	if err := service.ready(); err != nil {
		return time.Time{}, err
	}
	deviceID = strings.TrimSpace(deviceID)
	...
	if !service.store.TouchDevicePosture(deviceID, now) {
		return now, ErrNoPriorPosture
	}
	service.publish(events.TopicHealthChanged, map[string]string{
		"device_id": deviceID,
		"reason":    "device_posture_heartbeat",
	})
	return now, nil
}
