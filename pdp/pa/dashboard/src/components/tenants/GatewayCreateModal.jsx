...
import { copyText, formatDate, gatewayFQDN, gatewayLabelFromName, tenantDomain } from './tenantUtils';

export default function GatewayCreateModal({ tenant, form, setForm, error, enrollment, saving, onClose, onCreate }) {
  if (!tenant) return null;

  const gatewayDomain = tenantDomain(tenant);
  const gatewayDNSLabel = gatewayLabelFromName(form.name);
  const gatewayGeneratedFQDN = gatewayFQDN(form.name, tenant);
          ...
          <div className="bg-surface-card rounded-md border border-border shadow-2xl w-[min(560px,calc(100vw-32px))] overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div>
                ...
                <p className="text-[11px] text-text-muted mt-0.5">{tenant?.name || 'Tenant'}</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-[11px] font-semibold text-text-secondary uppercase tracking-[0.2px] mb-1.5">Tenant</label>
              ...
              <div>
                ...
                <div className="flex">
                  ...
                  <span className="inline-flex items-center max-w-[55%] px-3 py-2 bg-surface-secondary border border-l-0 border-border rounded-r-md text-[13px] text-text-secondary font-mono truncate">
                    {gatewayDomain ? `.${gatewayDomain}` : '.tenant-domain'}
                  </span>
                </div>
                ...
                <div className="p-3 border border-warning/30 rounded-md bg-warning-muted">
                  ...
                  <div className="grid gap-2 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <code className="text-mono text-text-primary">{formatDate(enrollment.expires_at)}</code>
                    </div>
                  </div>
                  <div className="mt-2">
            ...
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-border bg-surface-secondary">
              ...
              <button onClick={onCreate}
                disabled={saving || !tenant?.id || !gatewayDomain || !gatewayDNSLabel}
                className="px-4 py-2 text-xs font-semibold bg-accent text-white rounded-md hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors inline-flex items-center gap-1.5">
