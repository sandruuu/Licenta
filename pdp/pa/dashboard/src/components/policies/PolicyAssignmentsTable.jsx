...
export default function PolicyAssignmentsTable({ assignments, maps, onEdit, onDelete }) {
  return (
    <section className="rounded-md border border-border bg-surface-card shadow-surface">
      <div className="grid grid-cols-[190px_1.5fr_1.4fr_1.4fr_120px_120px] gap-4 border-b border-border bg-surface-secondary px-5 py-3 text-[10px] font-bold uppercase tracking-[0.14em] text-text-muted">
        <span>Type</span>
        <span>Policy</span>
      ...
      {assignments.length ? assignments.map((assignment) => {
        ...
        return (
          <div key={assignment.id} className="grid grid-cols-[190px_1.5fr_1.4fr_1.4fr_120px_120px] gap-4 border-b border-border px-5 py-4 last:border-b-0 hover:bg-surface-hover/60">
