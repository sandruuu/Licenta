...
func (s *tpmSigner) Sign(_ io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	...
	// ASN.1 DER encode the ECDSA signature
...
func asn1EncodeECDSASig(r, s *big.Int) []byte {
	rBytes := r.Bytes()
	sBytes := s.Bytes()
	// Pad with leading zero if high bit set
	...
	if blobData, err := os.ReadFile(blobPath); err == nil {
		...
		if err := json.Unmarshal(blobData, &blobs); err == nil {
			...
			if err == nil {
				...
				keyName = loadResp.Name
				pubKey, err = extractECCPub(tpmDev, keyHandle)
				if err == nil {
					slog.Info("Loaded existing TPM key", "path", blobPath)
					return &tpmSigner{tpm: tpmDev, handle: keyHandle, pub: pubKey, name: keyName}, nil
				}
			}
			slog.Warn("Failed to load TPM key blobs, creating new key", "error", err)
			// Remove stale blob file so it doesn't cause confusion on next restart
			os.Remove(blobPath)
		}
	}
