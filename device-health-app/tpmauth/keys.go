...
func NewKeyManager(dataDir string) (*KeyManager, error) {
	...
	if err == nil {
		perm := info.Mode().Perm()
		if perm&0077 != 0 {
			log.Printf("[TPM] Warning: data directory %s has permissive mode %o, expected 0700", dataDir, perm)
		}
	}
	...
	if err == nil {
		log.Printf("[TPM] Using TPM-backed key for mTLS")
		return &KeyManager{signer: signer, isTPM: true}, nil
	}
	log.Printf("[TPM] TPM not available (%v), using software key", err)
	...
	log.Printf("[TPM] Using software key for mTLS (no TPM)")
...
func loadOrCreateSoftwareKey(dataDir string) (crypto.Signer, error) {
	...
	log.Printf("[TPM] Generated new software ECDSA P-256 key: %s", keyPath)
	return key, nil
}
