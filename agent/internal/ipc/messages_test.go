...
func TestSubmitEnrollmentTokenRequestRoundTrip(t *testing.T) {
	sentAt := time.Now().UTC()
	request, err := NewRequest("req-1", OperationSubmitEnrollmentToken, SubmitEnrollmentTokenRequest{
	...
	var payload SubmitEnrollmentTokenRequest
	if err := DecodeBody(request.Body, &payload); err != nil {
		t.Fatalf("DecodeBody returned error: %v", err)
	}
	if payload.Token == "" || payload.Nonce != "nonce-1" || payload.KeyName != "ZTNA_DeviceKey_S-1-5-21-1" {
