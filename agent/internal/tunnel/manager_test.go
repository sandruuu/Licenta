...
func TestOpenResourceStreamIncludesStrictSessionFields(t *testing.T) {
	...
	manager := &Manager{
		options: Options{Timeout: time.Second, AccessTokenProvider: func() (string, string) {
	...
	stream, err := manager.OpenResourceStream(context.Background(), ResourceStreamRequest{TargetHost: "100.64.0.42", TargetPort: 22, SessionID: "sess-1", SessionToken: "session-token", ResourceID: "res-ssh", Protocol: "ssh", Process: &ProcessIdentity{PID: 42, Name: "ssh.exe"}})
	if err != nil {
