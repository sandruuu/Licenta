...
func NewManager(options Options) (*Manager, error) {
	...
	if options.GatewayAddress == "" {
		return nil, errors.New("gateway address is required when Gateway tunnel is enabled")
	}
...
func (manager *Manager) Run(ctx context.Context) error {
	...
	for {
		...
		if err := manager.Connect(ctx); err != nil {
			...
			manager.logger.Warn("ZTNA Agent Gateway tunnel connection failed", "error", err, "retry_in", backoff)
			select {
			case <-ctx.Done():
				manager.closeSession()
				manager.setStatus(Status{State: StatusStopped})
...
func (manager *Manager) OpenResourceStream(ctx context.Context, request ResourceStreamRequest) (net.Conn, error) {
	if manager == nil {
		return nil, errors.New("tunnel manager is nil")
	}
	if ctx == nil {
		ctx = context.Background()
	}
	...
	if manager.options.AccessTokenProvider != nil {
		accessToken, deviceID := manager.options.AccessTokenProvider()
		connectRequest.DeviceID = strings.TrimSpace(deviceID)
		if connectRequest.SessionID == "" || connectRequest.SessionToken == "" {
			connectRequest.Token = strings.TrimSpace(accessToken)
		}
	}
...
func (manager *Manager) connectLocked(ctx context.Context) error {
	...
	manager.logger.Info("ZTNA Agent Gateway tunnel connected", "gateway", manager.options.GatewayAddress)
	return nil
}
...
func (manager *Manager) helloLocked() error {
	...
	request := HelloRequest{Type: "hello", ClientVersion: ProtocolVersion, ClientApp: "ztna-agent", ClientBuild: clientBuild, Features: []string{"structured-codes", "hello-handshake"}}
