...
var ErrRefreshTokenUnavailable = errors.New("refresh token is not available")
...
func NewAuthenticator(cfg Config) (*Authenticator, error) {
	...
	if err != nil || issuerURL == nil || issuerURL.Scheme == "" || issuerURL.Host == "" {
		return nil, fmt.Errorf("issuer URL must be an absolute URL")
	}
	if issuerURL.Scheme != "https" {
		return nil, fmt.Errorf("issuer URL must use https")
	}
	...
	if cfg.Scopes == "" {
		return nil, fmt.Errorf("OIDC scopes are required")
	}
...
func (authenticator *Authenticator) Refresh(ctx context.Context) (*TokenSet, error) {
	authenticator.mu.Lock()
	defer authenticator.mu.Unlock()
	if authenticator.tokens == nil || strings.TrimSpace(authenticator.tokens.RefreshToken) == "" {
		return nil, ErrRefreshTokenUnavailable
	}
	return authenticator.refreshLocked(ctx)
}
...
func (authenticator *Authenticator) Authenticate(ctx context.Context, acrValues string) (*TokenSet, error) {
	...
	completeCallback := func(result callbackResult) {
		callbackOnce.Do(func() { codeCh <- result })
	}
	...
	mux.HandleFunc("/callback", func(w http.ResponseWriter, r *http.Request) {
		...
		if errParam := r.URL.Query().Get("error"); errParam != "" {
			completeCallback(callbackResult{err: fmt.Errorf("authorization error: %s", errParam)})
		...
		completeCallback(callbackResult{code: code})
	})
...
func (authenticator *Authenticator) tokenRequestLocked(ctx context.Context, form url.Values) (*TokenSet, error) {
	...
	if tokenResponse.TokenType != "" && !strings.EqualFold(tokenResponse.TokenType, "Bearer") {
		return nil, fmt.Errorf("token response returned unsupported token_type %q", tokenResponse.TokenType)
	}
