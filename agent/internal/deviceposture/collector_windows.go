...
func buildOSCheck(ctx context.Context) (ipc.DevicePostureCheck, string) {
	...
	return ipc.DevicePostureCheck{
		...
		Status:      ipc.DevicePostureStatusGood,
...
func buildFirewallCheck(ctx context.Context) (ipc.DevicePostureCheck, string) {
	status := collectFirewallStatus(ctx)
	check := ipc.DevicePostureCheck{
		Name: "Firewall",
		Details: map[string]string{
	...
	if status.AllEnabled {
		check.Status = ipc.DevicePostureStatusGood
		...
		check.Status = ipc.DevicePostureStatusWarning
		check.Description = "Some firewall profiles are disabled"
	} else {
		check.Status = ipc.DevicePostureStatusCritical
		check.Description = "Firewall is completely disabled"
	}
	return check, ""
}

func buildAntivirusCheck(ctx context.Context) (ipc.DevicePostureCheck, string) {
	info := collectAntivirusInfo(ctx)
	check := ipc.DevicePostureCheck{Name: "Antivirus", Details: map[string]string{}}
	if !info.Found {
		check.Status = ipc.DevicePostureStatusCritical
...
func buildDiskEncryptionCheck(ctx context.Context) (ipc.DevicePostureCheck, string) {
	...
	if info.IsEncrypted {
		...
		check.Status = ipc.DevicePostureStatusUnavailable
...
func buildPasswordCheck(ctx context.Context) (ipc.DevicePostureCheck, string) {
...
func buildWindowsUpdateCheck(ctx context.Context) (ipc.DevicePostureCheck, string) {
...
func buildConnectivityCheck(ctx context.Context) (ipc.DevicePostureCheck, string) {
...
if ($hotfix) {
  ...
  Write-Output ("LastInstalledOn: " + $hotfix.InstalledOn);
	...
	info := updateInfo{}
	output, err := commandOutput(ctx, "powershell", "-NoProfile", "-Command", script)
	if err != nil {
	...
	for _, line := range strings.Split(string(output), "\n") {
		line = strings.TrimSpace(line)
		switch {
		case strings.HasPrefix(line, "PendingUpdates"):
			info.PendingUpdates = extractAfterColon(line)
		case strings.HasPrefix(line, "LastHotFix"):
			info.LastHotFix = extractAfterColon(line)
		case strings.HasPrefix(line, "LastInstalledOn"):
			info.LastInstalledOn = extractAfterColon(line)
		}
	}
...
func collectConnectivityInfo(ctx context.Context) connectivityInfo {
	const targetHost = "www.msftconnecttest.com"
	info := connectivityInfo{Target: targetHost + ":80"}
	lookupCtx, cancelLookup := context.WithTimeout(ctx, 4*time.Second)
	addresses, err := net.DefaultResolver.LookupHost(lookupCtx, targetHost)
	cancelLookup()
	if err != nil {
	...
	connection, err := (&net.Dialer{Timeout: 4 * time.Second}).DialContext(dialCtx, "tcp", info.Target)
	cancelDial()
	if err != nil {
		info.LastError = err.Error()
		return info
