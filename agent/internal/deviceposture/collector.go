...
func (collector *Collector) Collect(ctx context.Context, deviceID string) (ipc.DevicePostureReport, error) {
	if ctx == nil {
...
type checkDefinition struct {
	Name  string
	Build func(context.Context) (ipc.DevicePostureCheck, string)
}

func collectChecks(ctx context.Context, budget time.Duration, definitions []checkDefinition) ([]ipc.DevicePostureCheck, string) {
	if len(definitions) == 0 {
		return nil, ""
	}
	checks := make([]ipc.DevicePostureCheck, len(definitions))
...
func runCheck(ctx context.Context, budget time.Duration, name string, build func(context.Context) (ipc.DevicePostureCheck, string)) (ipc.DevicePostureCheck, string) {
	if budget <= 0 {
		budget = defaultCollectorBudget
	}
