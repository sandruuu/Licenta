...
func TestValidStepUpURLRequiresTrustedHTTPSBrowserStepUpURL(t *testing.T) {
	...
	if !manager.validStepUpURL("https://pdp.example.test/browser/step-up/stepup-1") {
		t.Fatal("expected trusted PDP step-up URL to be accepted")
	}
	for _, raw := range []string{
		"http://pdp.example.test/browser/step-up/stepup-1",
		"https://evil.example.test/browser/step-up/stepup-1",
