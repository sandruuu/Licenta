...
../features/           Modul separat pentru capabilitati dormant ramase: TUN/Gateway/network
```
...
- `internal/service/protected-resources/*`: orchestreaza catalog -> resolver DNS local -> NRPT.
...
Capabilitatile de trafic real, TUN/Gateway routing si autorizare flow pe pachete raman inca separate in `../features` si nu sunt pornite de serviciul curent.
...
- `protected-resources`: porneste resolverul local, primeste catalogul autentificat si sincronizeaza resolverul + NRPT.
...
4. Catalogul contine:
   - `version`;
   - `dns_suffixes`, pastrate in payload pentru compatibilitate, dar nefolosite de agent la instalarea NRPT;
   - `resources`;
   - `ttl_seconds`;
   - `policy_epoch`.
5. `protected-resources` aplica intai catalogul in resolver.
6. Apoi instaleaza regulile NRPT pentru fiecare FQDN exact din `resources`.
7. Daca lista `resources` este goala, agentul nu instaleaza nicio regula NRPT.
7. Abia dupa ce pasii de mai sus reusesc, user-session devine `AUTHENTICATED`.
...
- activeaza DirectAccess pe regula cu `DAEnable`, pentru ca Windows sa includa regula in NRPT efectiv;
- seteaza `DANameServers = local_dns_server` si `NameServers = local_dns_server`;
- seteaza global `EnableDAForAllNetworks = EnableAlways`, `QueryPolicy = QueryBoth` si `SecureNameQueryFallback = FallbackPrivate`, ca regulile NRPT sa fie evaluate pe reteaua curenta;
