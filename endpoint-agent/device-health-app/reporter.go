...
func (cr *CloudReporter) Start() {
	...
	// Heartbeat: a 1-minute liveness ping that bumps the cloud-side
	// reported_at timestamp without resending the full health payload.
	// Pairs with the policy engine's posture-staleness check (10/30 min
	// thresholds in policy/risk.go); without this an idle but healthy
	// device would otherwise be downgraded to "stale posture" and start
	// accumulating risk after 10 minutes of inactivity.
	go cr.heartbeatLoop()
}

func (cr *CloudReporter) heartbeatLoop() {
	...
	for {
		...
		case <-cr.stopChan:
			return
		case <-ticker.C:
			req, err := http.NewRequest(http.MethodPost, cr.cloudURL+"/api/device/heartbeat", nil)
			if err != nil {
				continue
			}
			resp, err := cr.httpClient.Do(req)
			if err != nil {
				// Quiet: heartbeat failures are common during connectivity
				// blips and are visible via posture staleness anyway.
				continue
			}
			// 428 → cloud has no prior report; trigger a full report so
			// the next heartbeat will succeed.
