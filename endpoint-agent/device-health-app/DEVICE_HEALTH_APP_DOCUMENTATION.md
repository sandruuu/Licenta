...
Product boundary: Device Health App is the posture runtime of the unified Endpoint Agent documented in the [Endpoint Agent README](../README.md), sharing device identity and certificate state with Connect App.
...
- **Identitate**: shared endpoint identity module with TPM 2.0 ECDSA P-256 and software fallback
...
- **Dacă `cert_file` + `key_file` lipsesc** (mod enrollment):
  - Goroutine enrollment cu timeout **30 minute**
    - `EnrollAndWait(ctx, km, cloudURL, caFile, certSHA256, deviceID, hostname, dataDir)`
    - Generare shared device fingerprint, creare CSR cu component `endpoint`, browser auth, poll
    - Reutilizare certificat endpoint unic din shared state (`device.crt`, `ca.crt`) când Connect-App l-a emis deja
...
Device Health App uses the same shared endpoint identity implementation as Connect App through the `endpoint-agent/device-health-app/tpmauth` compatibility wrapper over `endpoint-agent/shared/endpointidentity`. New Cloud enrollment requests use component `"endpoint"`; Cloud still accepts legacy `"health"` and `"tunnel"` values by normalizing them to `"endpoint"`.

### 9.2 Key Manager (shared with Connect App)

- `NewKeyManager(dataDir)` → încearcă TPM 2.0, fallback software ECDSA P-256
- Cheia TPM/software, certificatul endpoint și starea de enrollment sunt salvate în același unified endpoint folder
- Rezolvare folder: `ZTNA_ENDPOINT_DIR`, apoi `ZTNA_TPM_DIR` ca alias legacy, apoi `%PROGRAMDATA%\ztna\endpoint` / `/var/lib/ztna/endpoint`, apoi fallback `data_dir`
- La prima rulare, `client.key` și `tpm-key.json` sunt copiate best-effort din vechiul folder `tpm` în unified endpoint folder dacă lipsesc din folderul nou
- Certificatul endpoint comun este salvat ca `device.crt` + `ca.crt`
...
1. **Verificare cache** (`device.crt` în unified endpoint folder):
...
5. **Cache pe disc**: `device.crt` + `ca.crt` (permisiuni 0600)

### 9.4 Auto-Renewal

#### `StartAutoRenewal(ctx, km, cloudURL, caFile, certSHA256, deviceID, hostname, dataDir)` — Goroutine

- **Ticker**: verificare la fiecare **1 oră**
- Per tick:
    - Citire `device.crt` → parsare `NotAfter`
