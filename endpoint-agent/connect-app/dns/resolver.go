...
func (r *Resolver) resolveInternal(ctx context.Context, name string) (string, int, error) {
	...
	// S5.1 — cap effective TTL at 90s so a revoked/moved resource is
	// re-resolved within a short, predictable window even when the
	// upstream policy hands out long TTLs.
	const maxTTL = 90
	if ttl <= 0 || ttl > maxTTL {
		ttl = maxTTL
	}
