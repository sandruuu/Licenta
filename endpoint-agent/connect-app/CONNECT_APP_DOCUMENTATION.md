...
Product boundary: Connect App is the network access runtime of the unified Endpoint Agent documented in the [Endpoint Agent README](../README.md), sharing device identity and certificate state with Device Health App.
...
- **Identitate dispozitiv**: shared endpoint identity module with TPM 2.0 hardware key and ECDSA P-256 software fallback
...
type Config struct {
    ...
    CAFile         string  // Certificat CA pentru verificarea serverului TLS
      CloudURL       string  // URL Cloud API pentru unified endpoint enrollment (ex. "https://cloud:8443")
      OIDCIssuerURL  string  // OIDC issuer for user auth; defaults to CloudURL
      OIDCClientID   string  // Public client ID; default "connect-app"
      OIDCScopes     string  // Default "openid profile email offline_access"
    DeviceID       string  // Identificator unic dispozitiv (opțional; derivat din TPM EK sau hostname)
      DataDir        string  // Fallback directory; shared key/cert cache uses machine-wide endpoint identity dirs when available
...
type activeConn struct {
   ...
   // TCP App-ID context captured at SYN time
   process          *tunnel.ProcessIdentity // Local process name/path/SHA-256/PID
}
```
...
- **Altfel** → flux TPM/Software enrollment:
   - Creare `KeyManager` din `endpoint-agent/shared/endpointidentity` cu fallback chain (TPM → software)
  - Generare device identity (hash TPM EK sau Windows MachineGuid)
   - `EnrollAndWait()` cu component `endpoint`, timeout 5 minute browser auth și server certificate pinning opțional via `cloud_cert_sha256`
   - Cache certificat unic endpoint în directorul shared state (`device.crt`, `ca.crt`), reutilizat de device-health-app
...
2. Captures TCP App-ID context from the Windows TCP owner table:
   - matches the local/remote TCP tuple to an owning PID
   - records process name, executable path and SHA-256 hash when available
   - stores the identity in `activeConn.process` so auth retries keep the same context
3. **Deschidere async stream yamux** (`context.WithTimeout(10s)`):
   - `tun_.OpenResourceStreamWithProcess(ctx, dstIP, dstPort, process)` → gateway deschide conexiune la resursă și primește process context
4. **Dacă stream reușește**:
...
5. **Dacă `ErrAuthRequired`**:
   - Setează `pendingAuth=true`, `pendingSyn=true`, `pendingClientISN=clientISN`
   - Declanșează browser OIDC direct către Cloud, fără să folosească un `auth_url` de la Gateway
...
## 8. User OIDC from Connect-App

### 8.1 Prezentare

Când gateway-ul returnează `auth_required`, Connect-App declanșează direct fluxul OIDC în browser ca **native public client**. Gateway-ul nu mai furnizează `auth_url` și nu mai este OIDC client. Particularitatea este că **handshake-ul TCP este amânat** — clientul nu primește SYN-ACK până când autentificarea nu se completează.

Current OIDC properties:

- `client_id=connect-app`
- Authorization Code + PKCE S256
- loopback redirect `http://127.0.0.1:<ephemeral>/callback`
- `device_id` and `hostname` are sent to Cloud `/auth/authorize`
- access token, `device_id`, and optional TCP process identity are attached to yamux `connect` frames
- refresh tokens are owned by Connect-App; Gateway never stores refresh tokens or OIDC client secrets
...
Client App          Connect App           Gateway           Cloud IdP
   ...
   │                    │←─loopback code─────│                  │  User authenticates
   │                    │──/auth/token─────────────────────────→│  PKCE exchange
   │                    │←─JWT───────────────│                  │
   │                    │──connect+token────→│                  │
    │                    │←─connected─────────│                  │
    │                    │                    │                  │
...
### 8.4 `triggerAuth(authErr)` — Native OIDC flow

- Pattern `sync.Once` — browser-ul se deschide o singură dată per sesiune de auth
- Uses `oidc.Authenticator.Authenticate(ctx, acrValues)` instead of trusting a gateway-provided URL
- Starts an ephemeral loopback HTTP listener on `127.0.0.1`
- Generates `state`, `nonce`, `code_verifier`, and PKCE S256 `code_challenge`
- Opens Cloud `/auth/authorize` with `device_id`, optional `hostname`, and optional `acr_values`
- Exchanges the returned code at Cloud `/auth/token`
- Stores tokens in memory and exposes the current access token via the tunnel auth provider
- Deschidere browser platform-specific:
...
### 8.6 Auth-flow unit tests

Connect-App unit tests now cover:

- native OIDC browser launch against Cloud `/auth/authorize`;
- loopback redirect validation on `http://127.0.0.1:<ephemeral>/callback`;
- PKCE S256 verifier/challenge consistency during `/auth/token` exchange;
- propagation of `device_id`, `hostname`, and MFA `acr_values` in the authorize URL;
- structured tunnel handling for `auth_required` and `mfa_required` responses without trusting a Gateway-provided `auth_url`.
...
`process` is optional. It is used as a Zero Trust policy/risk signal (App-ID), not as cryptographic proof; Gateway forwards it to Cloud PDP for conditions such as allowed process names and executable hashes.
...
Returnată de `OpenResourceStream()` și `ResolveDomain()` când gateway-ul necesită autentificare sau step-up. Consumată de handler-ul SYN din `main.go` pentru a declanșa fluxul OIDC public-client.
...
1. Deschide stream yamux nou
2. Atașează `token` și `device_id` de la providerul OIDC dacă există
3. Trimite `{"type": "connect", "remote_addr": "...", "remote_port": ..., "token": "...", "device_id": "..."}`
4. Așteaptă răspuns JSON
5. **La `auth_required` / `mfa_required`**: închide stream, returnează `ErrAuthRequired`
6. **La `connected`**: clearează deadline (stream-ul va fi folosit long-lived), returnează stream
7. Stream-ul devine tunel bidirecțional raw bytes
...
`endpoint-agent/connect-app/tpmauth` is now a compatibility wrapper over `endpoint-agent/shared/endpointidentity`. The same shared package is used by `endpoint-agent/device-health-app/tpmauth`, so both endpoint runtimes use one TPM/software key, one stable device identity, one certificate cache, and one Cloud enrollment record with component `endpoint`.

Shared endpoint identity artifacts are stored in one folder. Directory resolution order:
- `ZTNA_ENDPOINT_DIR` when set.
- `ZTNA_TPM_DIR` as a legacy alias when `ZTNA_ENDPOINT_DIR` is not set.
- Machine-wide defaults (`%PROGRAMDATA%\ztna\endpoint` on Windows, `/var/lib/ztna/endpoint` elsewhere).
- The configured `data_dir` only as a fallback when the machine-wide directory cannot be created.

On first use, existing `client.key` and `tpm-key.json` files are copied from the legacy key folder (`%PROGRAMDATA%\ztna\tpm` or `/var/lib/ztna/tpm`) into the unified endpoint folder when the new folder does not already contain them.
...
#### `loadOrCreateTPMKey(sharedKeyDir)` — Creare/încărcare cheie TPM
...
2. **Încercare TPM**: `loadOrCreateTPMKey(sharedEndpointDir)`
   - **Success**: return `KeyManager{signer, isTPM: true}`
3. **Fallback software**: `loadOrCreateSoftwareKey(sharedEndpointDir)`
...
type enrollmentRequest struct {
   ...
   Component            string  // "endpoint" (single shared endpoint certificate)
...
- Subject: `CN=deviceID`, `O="ZeroTrust Endpoint"`
- DNSNames: hostname
- Semnat cu signer-ul furnizat (TPM sau software)
- Return CSR PEM

#### `EnrollAndWait(ctx, km, cloudURL, caFile, certSHA256, deviceID, hostname, dataDir)` — Flux principal

1. **Verificare certificat cached** (`device.crt` în unified endpoint folder):
...
3. **Start sesiune browser**:
   ...
   - Body: `enrollmentRequest` cu `component: "endpoint"`
...
5. **Cache certificat pe disc**:
   - Scrie certPEM în `device.crt` (0600)
   - Scrie CAPEM în `ca.crt` (0600)

6. **Return `EnrollmentResult`**

#### `RenewCert(cloudURL, caFile, certSHA256, deviceID, csrPEM, fingerprint)`
- POST `/api/enroll/renew` — nou CSR cu fingerprint existent
- Returnează imediat cu certificat nou (fără polling)

#### `StartAutoRenewal(ctx, km, cloudURL, caFile, certSHA256, deviceID, hostname, dataDir)` — Goroutine background
- **Ticker**: verificare la fiecare **1 oră**
- Per tick:
   - Citire `device.crt` cached
...
| `ztna.local/shared` | local replace | Shared endpoint identity, TPM/software key, CSR, enrollment, renewal |
