...
type Options struct {
	...
	AccessEventRecorder func(contracts.AccessEvent)
...
func (forwarder *Forwarder) Sessions() []contracts.ActiveSession {
	if forwarder == nil {
		return nil
	}
	forwarder.mu.RLock()
	...
	sessions := make([]contracts.ActiveSession, 0, len(flows))
	for _, flow := range flows {
		flow.mu.Lock()
		session := contracts.ActiveSession{
			ID:         flow.id(),
...
func (forwarder *Forwarder) recordDenyFromSource(source string, mapping dnsresolver.Mapping, packet network.Packet, err error) {
	...
	forwarder.accessEventRecorder(contracts.AccessEvent{
